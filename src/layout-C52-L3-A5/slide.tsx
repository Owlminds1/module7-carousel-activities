...
import SlideData from "@/src/layout-C52-L3-A5/slideData.json";
      ...
      <div className="w-[90%] flex justify-center items-center flex-col gap-3">
            ...
            {SlideData.map((i, index) => (
              <SwiperSlide key={index}>
                <div className="grid grid-cols-12 w-full place-items-center p-2">
                  ...
                  <div className="col-span-6  w-full  flex flex-col  justify-center items-center gap-8">
                    <h3 className="text-xl font-bold text-black">
                      {i.question}
                    </h3>
                    <h3 className="text-2xl font-bold text-black">{i.title}</h3>
                    <div className="flex flex-wrap justify-center items-center gap-5">
                      {i.opt.map((btn, btnIndex) => (
                        <button
                          ...
                          key={btnIndex}
                          className={` ${
                            activeBtnIndex === btnIndex
                              ? btn === i.val
                                ? "bg-green-600"
                                : "bg-red-500 shake"
                              : "bg-violet-900"
                          } text-white px-5 py-2 cursor-pointer rounded-lg min-w-[270px]`}
                        >
                          {btn}
                        </button>
                      ))}
        ...
        <div className="flex justify-between w-full mt-8">
          ...
          <span
            onClick={handleNext}
            className={`${
              activeSlide < SlideData.length - 1 && show
                ? "visible"
                : "invisible"
            } cursor-pointer text-black text-4xl border border-black rounded-full p-3 bg-yellow-400`}
          >
